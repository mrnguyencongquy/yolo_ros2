...
[bên thứ 3: tiler]  (production)                     ┌─ container: ros2 (ROS2 Jazzy) ─────────────┐
  ...
  bbox LOCAL / JSON ◀── ZeroMQ REP ──────────│  YOLO inference (GPU)           │
        ...
        ▼
  /grass_segments (GrassSegmentationArray, bbox/polygon ảnh gốc)
        │
...
| `yolo_bridge` | ros2 | ✅ node | Cầu nối ROS2↔YOLO + **xử lý**: JPEG-encode, gọi ZMQ, đổi LOCAL→GLOBAL, gom theo `image_id`, publish `GrassSegmentationArray`. | `robot_ai/yolo_bridge.py` |
...
| `robot_ai_interfaces` | (build) | — | Định nghĩa msg `TileImage`, `GrassSegmentationArray`, `GrassSegment`, `BBox2D`. | `.../robot_ai_interfaces/msg/` |
...
| `segmentation_msg.py` | build `GrassSegmentationArray` | (colcon/E2E) |
...
5. Khi đủ `num_tiles` hoặc timeout, build `GrassSegmentationArray`.
6. Publish lên `/grass_segments`.

### 4.5 Output ROS2: `/grass_segments`

`/grass_segments` là output cho downstream. Có thể inspect bằng:
...
| Field | Ý nghĩa |
|---|---|
| `image_id` | ảnh/frame sinh ra segment |
| `segments[i].class_name` | tên class YOLO detect/segment được |
...
ROS2 Jazzy · Docker Compose · ZeroMQ (pyzmq) · Ultralytics YOLO · PyTorch CUDA · OpenCV · geometry_msgs · pytest/colcon.
