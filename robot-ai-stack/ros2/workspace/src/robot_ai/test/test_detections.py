...
def test_valid_list():
    raw = json.dumps([{"class_id": 0, "class_name": "target_class", "confidence": 0.9, "bbox": [1, 2, 3, 4]}]).encode()
    out = parse_detections(raw)
    assert out == [{"class_id": 0, "class_name": "target_class", "confidence": 0.9, "bbox": [1.0, 2.0, 3.0, 4.0], "polygon": []}]
...
def test_drops_detection_missing_bbox():       # TC-06
    raw = json.dumps([{"class_name": "target_class"}, {"class_id": 1, "class_name": "g", "confidence": 0.5, "bbox": [0, 0, 1, 1]}]).encode()
    out = parse_detections(raw)
